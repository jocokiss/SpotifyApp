...
class TestSpotifyApp(unittest.TestCase):
    ...
    @patch('spotipy.SpotifyOAuth')
    @patch('spotipy.SpotifyClientCredentials')
    @patch('spotipy.Spotify')
    def setUp(self, mock_spotify, mock_spotify_client_credentials, mock_spotify_oauth):
        self.mock_spotify = mock_spotify
        self.mock_spotify_oauth = mock_spotify_oauth
        self.mock_spotify_client_credentials = mock_spotify_client_credentials
        self.spotify_app = SpotifyApp()
        self.mock_tracks = [
            {'track': {'id': '1', 'name': 'Track 1', 'artists': ['Artist 1'], 'album': 'Album 1', 'explicit': False, 'popularity': 50, 'uri': 'uri1'}},
            {'track': {'id': '2', 'name': 'Track 2', 'artists': ['Artist 2'], 'album': 'Album 2', 'explicit': True, 'popularity': 60, 'uri': 'uri2'}}
        ]
